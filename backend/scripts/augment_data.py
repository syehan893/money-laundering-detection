...
def generate_smurfing_pattern(base_date: datetime, account_pool: list) -> list:
    ...
    for i, recv in enumerate(receivers):
        ...
        rows.append({
            ...
            "Sender_account": sender, "Receiver_account": recv,
            "Amount": amount, "Payment_currency": pay_curr,
            "Received_currency": recv_curr, "Sender_bank_location": sender_loc,
            "Receiver_bank_location": recv_loc, "Payment_type": "Wire",
            "Is_laundering": 1, "Laundering_type": "Smurfing",
        })
    return rows
...
def generate_round_tripping(base_date: datetime, account_pool: list) -> list:
    ...
    for i in range(len(accounts) - 1):
        ...
        rows.append({
            ...
            "Amount": tx_amount, "Payment_currency": CURRENCIES[sloc],
            "Received_currency": CURRENCIES[rloc], "Sender_bank_location": sloc,
            "Receiver_bank_location": rloc, "Payment_type": "Cross-border",
            "Is_laundering": 1, "Laundering_type": "Round_Tripping",
        })
    return rows
...
def generate_cross_border_layering(base_date: datetime, account_pool: list) -> list:
    ...
    for i in range(len(accounts) - 1):
        ...
        rows.append({
            ...
            "Received_currency": CURRENCIES[rloc], "Sender_bank_location": sloc,
            "Receiver_bank_location": rloc, "Payment_type": "Cross-border",
            "Is_laundering": 1, "Laundering_type": "Cross_Border_Layering",
        })
    return rows
...
def generate_fan_out_fan_in(base_date: datetime, account_pool: list) -> list:
    ...
    for i, mid in enumerate(intermediaries):
        ...
        rows.append({
            "Time": time_str, "Date": date_str,
            "Sender_account": hub_sender, "Receiver_account": mid,
            ...
            "Sender_bank_location": hub_loc, "Receiver_bank_location": mid_loc,
            "Payment_type": "Wire", "Is_laundering": 1,
    ...
    for i, mid in enumerate(intermediaries):
        ...
        rows.append({
            "Time": time_str, "Date": date_str,
            "Sender_account": mid, "Receiver_account": hub_receiver,
            ...
            "Sender_bank_location": mid_loc, "Receiver_bank_location": collect_loc,
            "Payment_type": "Cross-border", "Is_laundering": 1,
            "Laundering_type": "Fan_Out_Fan_In",
        })
